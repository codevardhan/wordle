...
function submitWordle() {
  ...
  for (let i = 0; i <= 4; i++) { 
    if (!targetMap.has(targetWord[i])) {
      targetMap[targetWord[i]] = 1;
    }
    else {
      targetMap[targetWord[i]] += 1;
    }
  ...
  for (let i = 0; i <= 4; i++) {
    let guessLetter = currentWord[i];
    let solutionLetter = targetWord[i];

    if (guessLetter === solutionLetter) {
      colourChanges[i] = "G";
    }
    else if (targetWord.indexOf(guessLetter) != -1 && targetMap[guessLetter]!=0) {
      colourChanges[i] = "Y";
      targetMap[guessLetter] -= 1;
    }
  }
  ...
  document.getElementById(`${currentRow}0`).focus();
}
...
function disableRow(rowID) {
  row = document.getElementsByClassName(`row${rowID}`);
  for (let i = 1; i <= 5; i++){
    row[i].disabled = true;
  }
}
...
function makeColourChanges(colourChanges) {
  row = document.getElementsByClassName(`row${currentRow}`);
  for (let i = 1; i <= 5; i++){
    row[i].classList.add(colourChanges[i-1]);
  }
}
